...
public class MotherboardTemperatureGraph : UserControl
{
    ...
    private async Task UpdateTemperaturesAsync(CancellationToken cancellationToken)
    {
        while (!cancellationToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(_motherboard.Temperature) && 
                    double.TryParse(_motherboard.Temperature.Replace(" °C", ""), out double currentTemperature))
                {
                ...
                {
                    Console.WriteLine("Motherboard temperature data is not available.");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error updating motherboard temperatures: {ex.Message}");
            }
